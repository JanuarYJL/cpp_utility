...
public:
    ...
    explicit ThreadPoolASIO(size_type thread_num = utility::workthreads_default)
        : work_guard_(asio::make_work_guard(io_svc_)), strand_(io_svc_)
    {
        if (thread_num < utility::workthreads_minnum)
        {
            thread_num = utility::workthreads_minnum;
    ...
    template <class F, class... Args>
    void StrandEnQueue(F &&f, Args &&... args)
    {
        io_svc_.post(strand_.wrap(td::bind(std::forward<F>(f), std::forward<Args>(args)...)));
    }

    template <class F, class... Args>
    auto StrandEnQueueFuture(F &&f, Args &&... args) -> std::future<typename std::result_of<F(Args...)>::type>
    {
        using return_type = typename std::result_of<F(Args...)>::type;

        auto temp_task_sptr_ = std::make_shared<std::packaged_task<return_type()>>(std::bind(std::forward<F>(f), std::forward<Args>(args)...));
        std::future<return_type> res = temp_task_sptr_->get_future();
        io_svc_.post(strand_.wrap([temp_task_sptr_]() {
            (*temp_task_sptr_)();
        }));
        return res;
    }
    ...
    template <class F, class... Args>
    void FullStrandEnQueue(F &&f, Args &&... args)
    {
        strand_.post(std::bind(std::forward<F>(f), std::forward<Args>(args)...));
    }

    template <class F, class... Args>
    auto FullStrandEnQueueFuture(F &&f, Args &&... args) -> std::future<typename std::result_of<F(Args...)>::type>
    {
        using return_type = typename std::result_of<F(Args...)>::type;

        auto temp_task_sptr_ = std::make_shared<std::packaged_task<return_type()>>(std::bind(std::forward<F>(f), std::forward<Args>(args)...));
        std::future<return_type> res = temp_task_sptr_->get_future();
        strand_.post([temp_task_sptr_]() {
            (*temp_task_sptr_)();
        });
        return res;
    }
